...
class AddGraphLearnedPositionalEmbeddings(PositionalEncoder):
    """
    Takes an encoded sequence and adds positional embeddings to it, which are learned jointly. Note that this will
    limited the maximum sentence length during decoding.
...
class ConcatGraphMLPPositionalEmbeddings(PositionalEncoder):
    """
    Takes an encoded sequence and concats positional embeddings to it, which are learned jointly. Note that this will
    limited the maximum sentence length during decoding.
    ...
    def __init__(self,
        ...
        #    self.embed_weight = mx.sym.Variable(prefix + "weight")
        #    self.embed
        self.embed_w1 = mx.sym.Variable(prefix + "1weight", shape=(1, num_embed))
        self.embed_w2 = mx.sym.Variable(prefix + "2weight", shape=(num_embed, num_embed))
        self.embed_b1 = mx.sym.Variable(prefix + "1bias", shape=(num_embed,))
        self.embed_b2 = mx.sym.Variable(prefix + "2bias", shape=(num_embed,))
    ...
    def encode(self,
        ...
        pos_embedding = mx.sym.Activation(data=pos_embedding,
                                          act_type='relu',
                                          name=self.prefix + "pos_embed_act_1")
        ...
        pos_embedding = mx.sym.Activation(data=pos_embedding,
                                          act_type='relu',
                                          name=self.prefix + "pos_embed_act_2")
        #pos_embedding = mx.sym.Embedding(data=positions,
        #                                 input_dim=self.max_seq_len,
        #                                 weight=self.embed_weight,
        #                                 output_dim=self.num_embed,
